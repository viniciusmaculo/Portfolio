...
export function ProjectDetails({ project, onClose }: ProjectDetailsProps) {
    ...
    return (
        <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center z-50" onClick={handleBackgroundClick}>
            <div className="bg-gradient-to-b from-[#302b63] to-[#313155] rounded-lg relative mx-3 max-w-xl p-5">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                    {project.technologies.map((item) => (
                        <p className="bg-orange-500 tracking-wide px-1 rounded">{item}</p>
                    ))}
                </div>
            </div>
        </div>
